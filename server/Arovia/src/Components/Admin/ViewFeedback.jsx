...
function ViewFeedback() {
  ...
  const getRatingIcon = (rating) => {
    if (rating >= 4) return 'fas fa-smile-wink'
    if (rating >= 3) return 'fas fa-smile'
    if (rating >= 2) return 'fas fa-meh'
    return 'fas fa-frown'
  }

  // Calculate average rating
  const averageRating = data.length > 0 
    ? (data.reduce((acc, curr) => acc + Number(curr.rating), 0) / data.length).toFixed(1)
    : 0
          ...
          {filteredData.length > 0 ? (
            <div className="feedback-grid">
              {currentItems.map((feedback, index) => {
                ...
                const ratingIcon = getRatingIcon(ratingValue);
                
                return (
                  <div key={feedback._id || index} className={`feedback-card ${ratingColor}`}>
                    ...
                    <div className="feedback-rating">
                      ...
                      <div className={`rating-badge ${ratingColor}`}>
                    ...
                    <div className="feedback-footer">
                      <div className="rating-info">
                        <span className={`rating-label ${ratingColor}`}>
                          {ratingLabel}
                        </span>
